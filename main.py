...
def get_dict_layer(passage_dict, title_dict, num):
    dockerfile_maneger = get_dockerfile_from_database.Dockerfile_Maneger("localhost", "test", num, 0, passwd = "123456") # ("211.69.198.51", "dockerfile", 1, 0) 
    dockerfile_list = dockerfile_maneger.lookup()
    for i in range(len(dockerfile_list)):
        image_id = dockerfile_list[i][0].replace("+","/")
...
def main():
    ...
    # 获取文章和标题的列表
    bad_num = 457
    get_dict_bad(passage_dict, title_dict, bad_num)         # 方式一：以 dockerfile 为节点
    get_dict(passage_dict, title_dict, num)         # 方式一：以 dockerfile 为节点
    get_dict_layer(passage_dict, title_dict, num)   # 方式二：以 dockerfile 的 layer 为节点
